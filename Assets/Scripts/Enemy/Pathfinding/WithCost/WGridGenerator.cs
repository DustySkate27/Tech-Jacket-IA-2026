...
[ExecuteInEditMode]
public class WGridGenerator : MonoBehaviour
{
    public PF_WNode prefab;
    public PF_WNode[] nodeGrid;
    public int width;
    public int height;
    ...
    {
        nodeGrid = new PF_WNode[width * height];
        for (int h = 0; h < height; h++)
        {
            for (int w = 0; w < width; w++)
            {
                PF_WNode newNode = Instantiate(prefab, transform.position
